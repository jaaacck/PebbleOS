...
// Only the start of the app identifier is hashed: the watch stores long identifiers truncated,
// and reverse-DNS identifiers are distinct well before this. A collision only keeps a
// notification that could have been removed.
...
static uint32_t prv_hash_app_id(const uint8_t *app_id, size_t length) {
...
// Active iOS notifications: from ANCS and not yet dismissed on either side
static bool prv_is_candidate(const SerializedTimelineItemHeader *header) {
  return header->common.ancs_notif && (header->common.type == TimelineItemTypeNotification) &&
         !(header->common.status & (TimelineItemStatusDismissed | TimelineItemStatusDeleted));
}
...
static bool prv_get_app_id_hash(const Uuid *id, uint32_t *hash_out) {
  ...
  *hash_out = prv_hash_app_id((const uint8_t *)app_id, strlen(app_id));
  timeline_item_free_allocated_buffer(&item);
  return true;
}

// Removing posts an event to KernelMain, which can only queue a few to itself, so remove one
// notification per callback
static void prv_remove_step(void *data) {
  RemoveContext *ctx = data;
  if (ctx->next >= ctx->count) {
    kernel_free(ctx);
    return;
  }

  Uuid *id = &ctx->ids[ctx->next++];
  notification_storage_remove(id);
  notifications_handle_notification_removed(id);
  launcher_task_add_callback(prv_remove_step, ctx);
}

//! Takes ownership of `ctx`
static void prv_remove_all(RemoveContext *ctx) {
  if (ctx->count == 0) {
    kernel_free(ctx);
    return;
  }
  PBL_LOG_INFO("Removing %u notifications cleared on the phone while disconnected",
               (unsigned)ctx->count);
  prv_remove_step(ctx);
}

static RemoveContext *prv_create_remove_context(size_t max_count) {
  RemoveContext *ctx = kernel_malloc_check(sizeof(RemoveContext) + max_count * sizeof(Uuid));
  *ctx = (RemoveContext){};
  return ctx;
}
...
void ancs_reconcile_entry_from_attributes(uint32_t uid, const ANCSAttribute *app_id,
  ...
  *entry_out = (ANCSReconcileEntry){
    .uid = uid,
    .app_id_hash =
        app_id ? prv_hash_app_id(app_id->value, app_id->length) : prv_hash_app_id(NULL, 0),
...
void ancs_reconcile_by_uid(const uint32_t *uids, size_t num_uids, const uint32_t *live_uids,
  ...
  CandidateList list = prv_get_candidates();
  RemoveContext *remove = prv_create_remove_context(list.count);
  for (size_t i = 0; i < list.count; i++) {
    const Candidate *candidate = &list.candidates[i];
    if (!prv_uid_in(candidate->ancs_uid, uids, num_uids) &&
        !prv_uid_in(candidate->ancs_uid, live_uids, num_live_uids)) {
      remove->ids[remove->count++] = candidate->id;
    }
  }
  kernel_free(list.candidates);
  prv_remove_all(remove);
}
...
void ancs_reconcile_by_content(const ANCSReconcileEntry *entries, size_t num_entries,
  ...
  if (num_rekeys) {
    // Later removals and actions from the phone use the new UIDs
